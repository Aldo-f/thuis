...
1. **Add import**: Insert `import signal` at the top of `src/thuis/main.py`.
...
2. **Register SIGINT handler**: Add `signal.signal(signal.SIGINT, lambda sig, frame: sys.exit("\nInterrupted by user"))` as the first statement inside `main()`.
...
3. **Create SIGINT test**: Add `tests/test_ctrl_c_handler.py` that launches the script, sends SIGINT, and asserts clean output.
...
4. **Update CI workflow**: Ensure the CI runs `pytest` so the new test is executed.
...
5. **Document the change**: Add a note to `README.md` describing the new Ctrl + C behavior.
