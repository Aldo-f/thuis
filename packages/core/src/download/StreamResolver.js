import { StreamDataSchema, StreamError, DrmError, GeoBlockedError, STREAM_ERROR_CODES, } from "./types.js";
 ...
 */
async function createPlayerInfoJwt() {
    ...
    const key = await crypto.subtle.importKey("raw", new TextEncoder().encode(JWT_SIGNING_KEY), { name: "HMAC", hash: "SHA-256" }, false, ["sign"]);
...
export class StreamResolver {
    ...
    async resolveStream(streamId) {
        const videoToken = await this.authService.getVideoToken().catch(() => {
            throw new StreamError("Video token niet beschikbaar. Log opnieuw in.", "NO_VIDEO_TOKEN");
        });
        ...
        if (!response.ok) {
            throw new StreamError(`Stream-verzoek mislukt (status ${response.status})`, "STREAM_FETCH_FAILED");
        }
    ...
    async getPlayerToken(videoToken) {
        ...
        if (!response.ok) {
            throw new StreamError(`Player token aanvraag mislukt (status ${response.status})`, "PLAYER_TOKEN_FAILED");
        }
        const json = await response.json();
        const token = json.vrtPlayerToken;
        if (!token) {
            throw new StreamError("Geen player token ontvangen.", "PLAYER_TOKEN_MISSING");
        }
        return token;
    }
    handleStreamErrorCode(code) {
        switch (code) {
            ...
            case STREAM_ERROR_CODES.LOGIN_REQUIRED:
                throw new StreamError("Je moet ingelogd zijn om deze video te bekijken.", "AUTH_REQUIRED");
        }
    }
}
