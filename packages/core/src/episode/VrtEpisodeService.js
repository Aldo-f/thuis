import { EpisodeDetailSchema } from "../types/episode.js";
...
export class VrtEpisodeService {
    ...
    async getEpisode(url) {
        ...
        if (!response.ok) {
            throw new EpisodeUnavailableError(`GraphQL-verzoek mislukt (status ${response.status})`);
        }
        const json = await response.json();
        // Check for GraphQL errors
        if (json.errors && json.errors.length > 0) {
            throw new EpisodeUnavailableError(`GraphQL-fout: ${json.errors[0].message}`);
        }
        ...
        if (!streamId) {
            throw new EpisodeUnavailableError("Geen stream ID gevonden voor deze aflevering.");
        }
